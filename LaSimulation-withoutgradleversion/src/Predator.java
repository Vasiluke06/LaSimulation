...
public class Predator extends Animals implements Movable {
    ...
    private static final int POINTS_FOR_EATING = 40;
    ...
    private Herbivore findClosestHerbivore() {
        ...
        for (Herbivore h : Simulation.herbivore) {
    ...
    private void moveToward(Herbivore target) {
        ...
        int dx = Integer.compare(target.getPosition().getX(), getPosition().getX())*STEP_SIZE;
        int dy = Integer.compare(target.getPosition().getY(), getPosition().getY())*STEP_SIZE;
    ...
    public void drawPredator(Graphics g){
        ...
        g.drawImage(Simulation.predator_img, p.getX(), p.getY(), 32, 32, null);
    }
